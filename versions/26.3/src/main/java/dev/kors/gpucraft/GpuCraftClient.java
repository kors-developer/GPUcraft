...
public final class GpuCraftClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        overlayKey = KeyMappingHelper.registerKeyMapping(new KeyMapping(
                "key.gpucraft.overlay", GLFW.GLFW_KEY_F9, category));
        presetKey = KeyMappingHelper.registerKeyMapping(new KeyMapping(
                "key.gpucraft.preset", GLFW.GLFW_KEY_F10, category));
        ...
        GpuCraft.LOGGER.info("GPUcraft v2 loaded for Minecraft 26.3, preset: {}", GpuCraft.config().preset.label());
    }

    public static String getOverlayKeyName() {
        if (overlayKey == null) return "F9";
        return formatKeyName(overlayKey.getTranslatedKeyMessage().getString());
    }

    public static String getPresetKeyName() {
        if (presetKey == null) return "F10";
        return formatKeyName(presetKey.getTranslatedKeyMessage().getString());
    }
    ...
    private static String formatKeyName(String name) {
        ...
        if (name.startsWith("key.mouse.")) {
            return "M" + name.substring("key.mouse.".length());
        }
        return name;
    }
