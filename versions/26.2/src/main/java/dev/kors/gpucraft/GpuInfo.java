...
/**
 * Какая видеокарта реально рендерит игру.
 *
 * Выбрать GPU из Java нельзя: это решает драйвер/ОС до старта JVM.
 * Поэтому мод показывает фактический адаптер, а переключение — в настройках системы.
 */
public final class GpuInfo {
    ...
    /** Вызывать только из рендер-потока. */
