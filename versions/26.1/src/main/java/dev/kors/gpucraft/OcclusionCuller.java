...
/**
 * Не рисуем то, что полностью закрыто блоками.
 *
 * Проверка — лучами от камеры к углам хитбокса. Она не бесплатная, поэтому
 * результат кешируется на occlusionIntervalMs: за это время сущность всё равно
 * не успеет заметно переместиться, а лучей пускается на порядок меньше.
 */
public final class OcclusionCuller {

    /** Ближе этого расстояния не проверяем вообще — дёшево и без мерцания вплотную. */
    ...
    /** @return true, если хотя бы один луч дошёл до сущности */
